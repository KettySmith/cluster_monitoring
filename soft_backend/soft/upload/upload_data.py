...
def upload_all_files(all_file_root_input: str = "2套集群数据"):
    ...
    for file_root in file_root_list:
        ...
        if (os.path.isdir(os.path.join(all_file_root, file_root))):
            upload_each_file(file_root, os.path.join(all_file_root, file_root), con, cursor)
        print("finish")
...
def upload_each_file(cluster_name: str, file_root_path: str, con, cursor):
    # 若数据库不存在，则根据集群名称创建数据库
    create_database(cluster_name, cursor)
    # 每个文件建立一张数据库表，递归创建
    dfs_create_table(file_root_path, cursor)


def create_database(database_name: str, cursor):
...
def dfs_create_table(file_root_path: str, cursor):
    count = 0
    for path, file_dir, files in os.walk(file_root_path):
        ...
        for file_name in files:
            # 创建数据表，并返回插入语句
            insert_sql = create_table(file_name, os.path.join(file_root_path, file_name), cursor)
            # 插入数据
            insert_data(insert_sql, os.path.join(file_root_path, file_name), cursor)
        # 为目录，递归创建
        for dir in file_dir:
            dfs_create_table(os.path.join(file_root_path, dir), cursor)
        count += 1


def create_table(table_name: str, file_path: str, cursor):
...
def insert_data(insert_sql: str, file_path: str, cursor):
