<template>
  <div class="chart" style="display: flex;">
    <div style="width:80%;">
      <div v-if="data_show" id="Transport_rx" style="width: 100%;height: 520px;margin-top: 50px;margin-bottom: 50px"></div>
...
export default {
  ...
  methods: {
    ...
    node_query() {
      ...
      axios.post('http://127.0.0.1:8090/show/get_node_single_data/' + this.cluster_name + '/elasticsearch_transport_rx_size_bytes_total',
        params, {
        headers: { 'content-type': 'application/x-www-form-urlencoded' }
      })//单指标和多指标都哟headers
        .then((res) => {
          //获取到数据
          let myChart = echarts.getInstanceByDom(document.getElementById("Transport_rx"));
          if(myChart==null){
            this.Transport_rx = echarts.init(document.getElementById('Transport_rx'))
          }
          else{
            this.Transport_rx.clear()
            this.Transport_rx = echarts.init(document.getElementById('Transport_rx'))
          }
          var option = {
            // title: {
        ...
        console.log(res.data)
          ...
          this.Transport_rx.setOption(option);
